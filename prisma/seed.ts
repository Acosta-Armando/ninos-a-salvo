...
const EDADES = [
  { label: "2 años", anios: 2 },
  { label: "4 años", anios: 4 },
  { label: "7 años", anios: 7 },
  { label: "9 años", anios: 9 },
  { label: "12 años", anios: 12 },
];
const RASGOS = [
...
async function main() {
  ...
  const count = 100;
  console.log(`Insertando ${count} niños de prueba sin nombre...`);
  ...
  const data = Array.from({ length: count }, () => {
    ...
    return {
      id: randomUUID(),
      fullname: null,
      edad_estimada: randomFrom(EDADES).label,
      edad_anios: randomFrom(EDADES).anios,
      nombre_padre:
        Math.random() > 0.7
          ? `Padre ref. ${Math.floor(Math.random() * 9000)}`
          : null,
      nombre_madre:
        Math.random() > 0.7
          ? `Madre ref. ${Math.floor(Math.random() * 9000)}`
          : null,
  ...
  console.log(`✓ ${result.count} registros creados con foto /favicon.ico`);
