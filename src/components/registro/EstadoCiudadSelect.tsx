  ...
  return (
    <div className='grid gap-4 sm:grid-cols-2'>
      ...
      <div className='space-y-2'>
        ...
        <Select value={ciudad || undefined} onValueChange={onCiudadChange} disabled={!estado} required={required}>
          <SelectTrigger className='w-full'>
            <SelectValue placeholder={estado ? 'Seleccionar municipio' : 'Primero elige un estado'} />
          </SelectTrigger>
          <SelectContent>
            {municipios.map((name) => (
              <SelectItem key={name} value={name}>
